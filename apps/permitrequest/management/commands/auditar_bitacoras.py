...
class Command(BaseCommand):
    help = 'Audita bitácoras de SIGETH1 y muestra detalles de errores'

    def add_arguments(self, parser):
        parser.add_argument('--anio', type=str, default='2026', help='Año a auditar')
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                sql = """
                      SELECT per.cedula, p.registered_by, p.action, p.date_permission_start, p.start_time, p.end_time
                ...
                while True:
                    ...
                    for cedula, reg_by, accion, fecha, h_ini, h_fin in rows:
                        total_periodo += 1
            ...
            # MOSTRAR CÉDULAS ESPECÍFICAS
            if cedulas_faltantes:
                self.stdout.write(self.style.ERROR(f"❌ CÉDULAS NO ENCONTRADAS EN SIGETH2 ({len(cedulas_faltantes)}):"))
                for c in cedulas_faltantes:
                    self.stdout.write(self.style.NOTICE(f"   -> {c}"))

            if tipos_faltantes:
                self.stdout.write(self.style.ERROR(f"❌ TIPOS FALTANTES: {tipos_faltantes}"))

            if usuarios_faltantes:
                self.stdout.write(self.style.WARNING(f"⚠️  USUARIOS SIN COINCIDENCIA: {len(usuarios_faltantes)}"))
