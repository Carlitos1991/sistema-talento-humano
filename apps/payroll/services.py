...
class PayrollCalculatorService:
    ...
    def _prepare_mass_data(self, emp_ids):
        """
        Extrae todos los datos pesados (Feriados, Permisos, Días del mes anterior)
        en una sola consulta para evitar saturar la base de datos (N+1 queries).
        """
        # 1. Feriados del mes actual
        ...
        # 2. Días efectivos laborados en el periodo anterior (Para beneficios CT a mes vencido)
        ...
        # 3. Permisos Aprobados del mes actual (Para descontar días efectivos)
        ...
        for permit in approved_permits:
            ...
            # Descontar el día si el permiso es de >= 1 día, >= 8 horas, o dura varios días
        ...
        with transaction.atomic():
            ...
            for slip in created_payslips:
                try:
                    ...
                    # =================================================================
                    # CALCULATE EFFECTIVE WORKED DAYS (Fines de semana, Feriados y Permisos)
                    # =================================================================
                    effective_days = 0
                        ...
                        while curr_date <= tramo['real_end']:
                            if curr_date.weekday() < 5:
                                if curr_date not in holiday_dates:
                                    if curr_date not in emp_absences:
                                        effective_days += 1
                    ...
                    # --- EVALUACIÓN DE INGRESOS (Beneficios y Fórmulas) ---
                    for inc in active_incomes:
                        ...
                        # Beneficios Código de Trabajo (Solo pagan si anios_servicio >= 1)
                            ...
                            if regime_code == 'CT' and anios_servicio >= 1:
                                ...
                                val = daily_food_allowance * Decimal(str(effective_days_prev))
                        ...
                        elif code_clean == 'SUBSIDIO_FAMILIAR':
                            if regime_code == 'CT' and anios_servicio >= 1:
                                if num_hijos_validos > 0:
                                    sbu = Decimal(str(self.config.get('SBU', '460.00')))
                                    pct_familiar = Decimal('1.00') / Decimal('100.0')
                                    val = sbu * pct_familiar * Decimal(str(num_hijos_validos))
            ...
            # --- ASIGNACIÓN DE PARTIDAS A ITEMS ---
    ...
    def generate_for_selected(self, employees_with_days):
        ...
        with transaction.atomic():
            ...
            # --- AGREGACIÓN CONTABLE ---
            # Si solo se recalcula para unos empleados no deberíamos sobreescribir TODO el asiento contable general
            # pero por mantener la lógica original de tu método:
            warnings = self._generate_accounting_journal(created_payslips, partial=True)

            return {"success": True, "warnings": warnings}

    def _assign_budget_lines_to_items(self, created_payslips, assignment_map):
        """Método auxiliar para evitar código duplicado de asignación de partidas presupuestarias a los rubros"""
        try:
    ...
    def _generate_accounting_journal(self, created_payslips, partial=False):
        """Método auxiliar para evitar código duplicado de la jornalización contable"""
        if partial:
            rebuild_accounting_for_period(self.period.id)
            return []  # Retornamos lista vacía de advertencias

        aggregation = {}
        budget_aggregation = {}
        warnings = []

        # Obtenemos TODOS los items del periodo si es un cálculo parcial, o solo los creados si es masivo
        # Para que no se descuadre la contabilidad al recalcular solo a 1 persona
        if partial:
            all_payslips_in_period = Payslip.objects.filter(period=self.period)
            items_qs = PayslipItem.objects.filter(payslip__in=all_payslips_in_period).select_related(
                'income_ref', 'deduction_ref', 'contribution_ref'
            )
            total_net_pay = sum(Decimal(str(slip.net_pay)) for slip in all_payslips_in_period)
        else:
...
def rebuild_accounting_for_period(period_id):
    """
    Reconstruye el Asiento Contable sumando directamente en la Base de Datos (Ultra rápido).
    """
    from decimal import Decimal
    from django.db import transaction
    ...
    with transaction.atomic():
        ...
        # 1. Sumar totales directo en la BD (Milisegundos en lugar de minutos)
        total_net_pay = payslips.aggregate(t=Sum('net_pay'))['t'] or Decimal('0.0')

        aggregation = {}

        # INGRESOS DEBE
        inc_debit = PayslipItem.objects.filter(
            payslip__period=period, item_type='INCOME', income_ref__debit_account__isnull=False
        ).values('income_ref__debit_account_id', 'budget_line_code').annotate(total=Sum('value'))
        for row in inc_debit:
            key = (row['income_ref__debit_account_id'], row['budget_line_code'], 'debit')
            aggregation[key] = aggregation.get(key, Decimal('0.0')) + (row['total'] or Decimal('0.0'))

        # INGRESOS HABER
        inc_credit = PayslipItem.objects.filter(
            payslip__period=period, item_type='INCOME', income_ref__credit_account__isnull=False
        ).values('income_ref__credit_account_id', 'budget_line_code').annotate(total=Sum('value'))
        for row in inc_credit:
            key = (row['income_ref__credit_account_id'], row['budget_line_code'], 'credit')
            aggregation[key] = aggregation.get(key, Decimal('0.0')) + (row['total'] or Decimal('0.0'))

        # EGRESOS DEBE
        ded_debit = PayslipItem.objects.filter(
            payslip__period=period, item_type='DEDUCTION', deduction_ref__debit_account__isnull=False
        ).values('deduction_ref__debit_account_id').annotate(total=Sum('value'))
        for row in ded_debit:
            key = (row['deduction_ref__debit_account_id'], None, 'debit')
            aggregation[key] = aggregation.get(key, Decimal('0.0')) + (row['total'] or Decimal('0.0'))

        # EGRESOS HABER
        ded_credit = PayslipItem.objects.filter(
            payslip__period=period, item_type='DEDUCTION', deduction_ref__credit_account__isnull=False
        ).values('deduction_ref__credit_account_id').annotate(total=Sum('value'))
        for row in ded_credit:
            key = (row['deduction_ref__credit_account_id'], None, 'credit')
            aggregation[key] = aggregation.get(key, Decimal('0.0')) + (row['total'] or Decimal('0.0'))

        # APORTES DEBE Y CUENTAS PUENTE PATRONAL
        con_debit = PayslipItem.objects.filter(
            payslip__period=period, item_type='CONTRIBUTION', contribution_ref__debit_account__isnull=False
        ).values('contribution_ref__debit_account_id', 'contribution_ref__code').annotate(total=Sum('value'))
        ...
        for row in con_debit:
            val = row['total'] or Decimal('0.0')
            key = (row['contribution_ref__debit_account_id'], None, 'debit')
            aggregation[key] = aggregation.get(key, Decimal('0.0')) + val

            if 'PATRONAL' in (row['contribution_ref__code'] or '').upper() and cta_gp_id:
        ...
        # APORTES HABER
        con_credit = PayslipItem.objects.filter(
            payslip__period=period, item_type='CONTRIBUTION', contribution_ref__credit_account__isnull=False
        ).values('contribution_ref__credit_account_id').annotate(total=Sum('value'))
        for row in con_credit:
            key = (row['contribution_ref__credit_account_id'], None, 'credit')
            aggregation[key] = aggregation.get(key, Decimal('0.0')) + (row['total'] or Decimal('0.0'))

        # 2. Generación del Asiento
        if aggregation or total_net_pay > 0:
            journal = Journal.objects.create(date=period.end_date, description=desc_asiento)
            total_debits = Decimal('0.0')
            total_credits = Decimal('0.0')

            # Traemos las cuentas de una vez para evitar múltiples queries
            acc_ids = set(k[0] for k in aggregation.keys())
            accounts_map = {a.id: a for a in Account.objects.filter(id__in=acc_ids)}
                ...
                if acc:
                    if mov_type == 'debit':
                        journal_items_to_create.append(
                            JournalItem(journal=journal, account=acc, debit=val, credit=Decimal('0.0')))
                        total_debits += val
                    else:
                        journal_items_to_create.append(
                            JournalItem(journal=journal, account=acc, debit=Decimal('0.0'), credit=val))
                        total_credits += val
            ...
            if total_net_pay > 0:
                try:
                    ...
                    cta_banco = Account.objects.get(code='1.1.1.03.01')
                    journal_items_to_create.append(
                        JournalItem(journal=journal, account=cta_gastos_personal, debit=total_net_pay,
                                    credit=Decimal('0.0')))
                    total_debits += total_net_pay
                    journal_items_to_create.append(
                        JournalItem(journal=journal, account=cta_banco, debit=Decimal('0.0'), credit=total_net_pay))
                    total_credits += total_net_pay
                except Account.DoesNotExist:
                    pass
            ...
            if total_debits != total_credits:
                ...
                if balancing_account:
                    if diff > 0:
                        journal_items_to_create.append(
                            JournalItem(journal=journal, account=balancing_account, debit=Decimal('0.0'), credit=diff))
                    else:
                        journal_items_to_create.append(
                            JournalItem(journal=journal, account=balancing_account, debit=abs(diff),
                                        credit=Decimal('0.0')))
