class ContractTemplateEditor {
    ...
    getAvailableMappings() {
        const dataNode = document.getElementById('contractTemplateEditorMappingsData');
        if (!dataNode) return [];
        try {
    ...
    attachEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (textarea) {
                this.wrapSelection(textarea, formatBtn.dataset.prefix || '', formatBtn.dataset.suffix || '');
            }
        });

        this.sectionEditorModal?.addEventListener('click', (e) => {
            if (e.target === this.sectionEditorModal) e.preventDefault();
        });
    ...
    openVariablesAlert() {
        ...
        const rows = mappings.map((item) => {
            ...
            return `<div class="template-editor-var-item"><code>${placeholder}</code><span>${label}</span></div>`;
        }).join('');

        const content = rows || '<div class="template-editor-var-empty">No hay variables configuradas.</div>';
    ...
    openSectionEditorModal(mode, sectionId = null) {
        ...
        if (mode === 'edit' && sectionId) {
            ...
            this.sectionEditorTitle.textContent = 'Editar Párrafo';
            ...
            this.sectionEditorTitle.textContent = 'Agregar Nuevo Párrafo';
            this.sectionEditorContent.value = '';
            if (this.saveSectionBtn) this.saveSectionBtn.innerHTML = '<i class="fas fa-plus"></i> Agregar Párrafo';
        }
    ...
    createSection(content) {
        ...
        this.fetchJson(`/contract/templates/${this.templateId}/sections/create/`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json', 'X-CSRFToken': this.csrfToken},
            body: JSON.stringify({section_type: 'PARAGRAPH', content: content, order: maxOrder}),
        }).then((data) => {
            if (data.success) {
                ...
                this.updatePreview();
                this.closeModal(this.sectionEditorModal);
                this.showNotification('Párrafo agregado exitosamente', 'success');
            }
        }).catch((err) => {
            console.error(err);
    ...
    updateSection(sectionId, content) {
        this.fetchJson(`/contract/templates/sections/${sectionId}/update/`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json', 'X-CSRFToken': this.csrfToken},
            body: JSON.stringify({section_type: 'PARAGRAPH', content: content}),
        }).then((data) => {
            if (data.success) {
                ...
                this.updatePreview();
                this.closeModal(this.sectionEditorModal);
                this.showNotification('Párrafo actualizado', 'success');
            }
        }).catch((err) => {
            console.error(err);
            this.showNotification(err.message || 'Error de red', 'danger');
    ...
    handleDeleteSection(sectionId) {
        const executeDelete = () => {
            this.fetchJson(`/contract/templates/sections/${sectionId}/delete/`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json', 'X-CSRFToken': this.csrfToken},
            }).then((data) => {
                if (data.success) {
                    ...
                    this.toggleEmptyState();
                    this.updatePreview();
                    this.showNotification('Párrafo eliminado', 'success');
                }
            }).catch((err) => {
                console.error(err);
                this.showNotification(err.message || 'Error de red', 'danger');
        ...
        if (confirm('¿Eliminar este párrafo?')) executeDelete();
    }
    ...
    initDragDrop() {
        ...
        document.addEventListener('drop', (e) => {
            e.preventDefault();
            if (draggedElement) this.saveReorder();
        });
    }
    ...
    saveReorder() {
        ...
        items.forEach((item, index) => {
            sections.push({id: parseInt(item.dataset.sectionId, 10), order: index});
        });

        this.fetchJson(`/contract/templates/${this.templateId}/sections/reorder/`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json', 'X-CSRFToken': this.csrfToken},
    ...
    updatePreview() {
        this.fetchJson(`/contract/templates/${this.templateId}/preview/`).then((data) => {
            const preview = document.getElementById('preview');
            if (!preview) return;
            if (data.preview) preview.innerHTML = data.preview;
            else if (data.error) preview.innerHTML = `<div class="alert alert-danger">${data.error}</div>`;
        }).catch((err) => {
            console.error(err);
            const preview = document.getElementById('preview');
            if (preview) {
                preview.innerHTML = this.buildLocalPreviewHtml(err);
            }
        });
    }

    buildLocalPreviewHtml(error = null) {
        const sections = this.collectLocalSectionsForPreview();
        const body = sections.length
            ? sections.join('')
            : '<p style="text-align:justify; margin-bottom:0.6rem; line-height:1.42; font-size:0.95rem;">Agregue párrafos en el editor para visualizar el cuerpo del contrato.</p>';
        ...
        return `
            <div style="max-width: 800px; margin: 1rem auto; padding: 2.4rem 2rem 1.4rem; border: 1px solid #ddd; background: white;">
                ...
                <div style="text-align:left; margin-bottom:1.1rem; font-size:0.92rem;">Loja, ${new Date().toLocaleDateString('es-EC')}</div>
                <div class="template-sections">
                    ${body}
                    ${error ? `<p class="text-muted small mt-3">${this.escapeHtml(error.message || 'Error interno del servidor')}</p>` : ''}
                </div>
            </div>
        `;
    }

    collectLocalSectionsForPreview() {
        const items = Array.from(document.querySelectorAll('.section-item'));
        return items.map((item) => {
            const rawType = item.dataset.sectionType || 'PARAGRAPH';
            const content = this.escapeHtml(this.decodeEscapedContent(item.dataset.sectionContent || ''));
            if (rawType === 'TITLE') {
                return `<h4 style="text-align:left; margin-top:0.9rem; margin-bottom:0.35rem; font-size:1rem;">${content}</h4>`;
            }
            return `<p style="text-align:justify; margin-bottom:0.6rem; line-height:1.42; font-size:0.95rem;">${content}</p>`;
        });
    }

    showNotification(message, type = 'info') {
        const iconByType = {success: 'success', danger: 'error', warning: 'warning', info: 'info'};
        if (typeof Swal !== 'undefined') {
            const toast = Swal.mixin({toast: true, position: 'top-end', showConfirmButton: false, timer: 2500, timerProgressBar: true});
            toast.fire({icon: iconByType[type] || 'info', title: message});
            return;
        }
    }
    ...
    appendSection(section) {
        ...
        const displayType = section.section_type || (rawType === 'TITLE' ? 'Título' : 'Párrafo');
            ...
            <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    ...
                    <div class="btn-group btn-group-sm" role="group">
                        <button type="button" class="btn btn-outline-primary btn-edit" data-section-id="${section.id}" title="Editar"><i class="fas fa-pencil"></i></button>
                        <button type="button" class="btn btn-outline-danger btn-delete" data-section-id="${section.id}" title="Eliminar"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
    ...
    updateSectionItem(section) {
        ...
        if (contentNode) contentNode.innerHTML = this.escapeHtml(normalizedContent).replace(/\n/g, '<br>');
    }
    ...
    updateOrderLabels() {
        ...
        items.forEach((item, index) => {
            ...
            if (orderNode) orderNode.textContent = `Orden: ${index}`;
        });
    }
