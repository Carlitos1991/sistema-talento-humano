...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updatePaginationUI(totalRows, totalPages) {
        ...
        if (pageInfo) {
            ...
            pageInfo.textContent = `Mostrando ${start} a ${end} registros de ${totalRows} registros`;
        }
    ...
    window.filterByStatus = function (status) {
        ...
        [cardAll, cardActive, cardInactive].forEach(card => {
            if (card) card.classList.remove('opacity-low');
        });

        if (status === 'all' && cardAll) {
            // No hacer nada, todas las tarjetas visibles
        } else if (status === 'true' && cardActive) {
            if (cardAll) cardAll.classList.add('opacity-low');
            if (cardInactive) cardInactive.classList.add('opacity-low');
        } else if (status === 'false' && cardInactive) {
            if (cardAll) cardAll.classList.add('opacity-low');
            if (cardActive) cardActive.classList.add('opacity-low');
        }
    };
    ...
    // =========================================================
    // 7. EVENT LISTENERS (Búsqueda, Paginación, Botón Nuevo)
    // =========================================================
    ...
    if (btnAddCatalog && window.vmCatalog) {
        btnAddCatalog.addEventListener('click', () => {
            window.vmCatalog.openCreate();
        });
    }
